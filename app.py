...
def score_short_detail(row):
    ...
    return [
        ("RSI", linear(rsi, 60, 30, 15), 15),
        ("MACD", 15 if row.get('MACD_GC', 0) == 1 else 0, 15),
        ("52w下落", linear(drop, 0.05, 0.30, 15), 15),
        ("出来高", linear(vr, 1.0, 2.0, 10), 10),
        ("予兆", linear(pc, 0, 3, 10), 10),
        ("トレンド", 10 if p > m50 > m200 and m50 > 0 else 0, 10),
        ("決算", earn_score(d, 10), 10),
...
def score_long_detail(row):
    ...
    per = row.get('PER', 0)
    return [
        ("ROE", linear(row.get('ROE', 0), 0.05, 0.20, 12), 12),
        ("PER", linear(per, 25, 10, 12) if per > 0 else 0, 12),
        ("利益率", linear(row.get('利益率', 0), 0.05, 0.20, 10), 10),
        ("FCF", linear(row.get('FCFマージン', 0), 0, 0.15, 10), 10),
        ("52w下落", linear(drop, 0.05, 0.35, 10), 10),
        ("粗利率", linear(row.get('粗利率', 0), 0.20, 0.50, 8), 8),
        ("RSI", linear(rsi, 60, 30, 8), 8),
        ("MACD", 8 if row.get('MACD_GC', 0) == 1 else 0, 8),
        ("決算", earn_score(d, 7), 7),
        ("EPS", 5 if row.get('EPS', 0) > 0 else 0, 5),
...
if st.session_state.selected_stock is not None:
    ...
    if not rr.empty:
        ...
        st.markdown(f"### {ml} 総合スコア: **{total_score}/100点**")
...
# =============================================================================
    ...
    if not fdf.empty:
        def calc_row(row):
            ...
            if is_short:
                return pd.Series([
                    total, drop_str, rsi_str, er,
                    score_strs.get("RSI", ""), score_strs.get("MACD", ""),
                    score_strs.get("52w下落", ""), score_strs.get("出来高", ""),
                    score_strs.get("予兆", ""), score_strs.get("トレンド", ""),
                    score_strs.get("決算", ""), score_strs.get("EPS", ""),
                    score_strs.get("ROE", ""), score_strs.get("利益率", ""),
                ])
            else:
                return pd.Series([
                    total, drop_str, rsi_str, er,
                    score_strs.get("ROE", ""), score_strs.get("PER", ""),
                    score_strs.get("利益率", ""), score_strs.get("FCF", ""),
                    score_strs.get("52w下落", ""), score_strs.get("粗利率", ""),
                    score_strs.get("RSI", ""), score_strs.get("MACD", ""),
                    score_strs.get("決算", ""), score_strs.get("EPS", ""),
                    score_strs.get("利益質", ""), score_strs.get("トレンド", ""),
                ])

        if is_short:
            col_names = ['スコア', '52w位置', '過熱感', '決算リスク',
                         'RSI /15', 'MACD /15', '52w下落 /15', '出来高 /10',
                         '予兆 /10', 'トレンド /10', '決算 /10',
                         'EPS /5', 'ROE /5', '利益率 /5']
        else:
            col_names = ['スコア', '52w位置', '過熱感', '決算リスク',
                         'ROE /12', 'PER /12', '利益率 /10', 'FCF /10',
                         '52w下落 /10', '粗利率 /8', 'RSI /8', 'MACD /8',
                         '決算 /7', 'EPS /5', '利益質 /5', 'トレンド /5']
        ...
        display_cols = (['順位', '記号', '銘柄', 'スコア', '52w位置', '過熱感', '決算リスク',
                         '次回決算日', '配当日']
                        + [c for c in col_names if c not in ['スコア', '52w位置', '過熱感', '決算リスク']]
                        + ['株価'])
